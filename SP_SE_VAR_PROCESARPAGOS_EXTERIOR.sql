...
DECLARE DECMONTOIMPUESTO_TOTAL DECIMAL (18, 4) DEFAULT 0;
DECLARE DECTASASEGURIDAD_TOTAL DECIMAL (18, 4) DEFAULT 0;
DECLARE DECNUMMONTOCOMISION_TOTAL DECIMAL (18, 4) DEFAULT 0;
...
SET CHRPROSEGUIR = 'N';
IF ABS (INTTIPORESULTADO) < 10000 THEN
SET VARMENSAJERESULTADO = BLHSRVLIB.FN_SE_SEL_ERROR (
        LPAD (
            TRIM (
                CAST (ABS (INTTIPORESULTADO) AS VARCHAR (10))
            ),
            4,
            '0'
        )
...
ELSE
SET VARMENSAJERESULTADO = 'ERROR INTERNO: ' || TRIM (CAST (INTTIPORESULTADO AS VARCHAR (20)));
END IF;
END;
...
SELECT DATE (
        DIGITS (CAST (CNTRDY + 2000 AS DEC (4, 0))) || '-' || DIGITS (CAST (CNTRDM AS DEC (2, 0))) || '-' || DIGITS (CAST (CNTRDD AS DEC (2, 0)))
    ) INTO DATFECHAACTUAL
...
SET VARMENSAJERESULTADO = BLHSRVLIB.FN_SE_SEL_ERROR (
        LPAD (
            TRIM (CAST (INTTIPORESULTADO AS VARCHAR (10))),
            4,
            '0'
        )
    );
...
SET CHRPROSEGUIR = 'N';
END IF;
IF CHRPROSEGUIR = 'S' THEN --SET VarMensajeResultado = '' ; 
--** Ciclo de facturas 
...
ORDER BY F.CTAPAGADOR,
    ...
    F.FORMAPAGO,
    F.MONEDAFACTURA,
    COALESCE (AA.TIPO, 1) DO
SET CHRPROSEGUIR = 'S';
SET INTTIPORESULTADO = 0;
SET VARMENSAJERESULTADO = 'PROCESO INICIADO';
...
SET VARMENSAJERESULTADO = 'CTA PAGADOR: ' || TRIM (CAST (NUMCTAPAGADOR_ANT AS VARCHAR (20))) || ' - CTA BENEFICIARIO: ' || COALESCE (CHRCUENTABENEFICIARIO, '');
...
SET VARMENSAJERESULTADO = 'CIF PAGADOR: ' || TRIM (CAST (NUMCIFPAGADOR AS VARCHAR (20))) || ' - CIF BENEFICIARIO: ' || TRIM (CAST (NUMCIFBENEFICIARIO AS VARCHAR (20)));
INSERT INTO SESSION.PGLOGPROCESOTMP (
        FECHAINSERCION,
        NUMEROGRUPO,
        CODTRANSACCION,
...
CALL BLHSRVLIB.SP_SE_VAR_TRANSFERENCIAEXTERIOR (
    ...
    DECTCUSDCOMPRA,
    DECTCUSDVENTA,
    DECMONTODEBITOGASTOCORRESPONSAL,
    DECMONTODEBITOCOMISION,
    DECTASASEGURIDAD_TOTAL,
    CHRTIPOBANCOINTERM,
    CHRBANCOINTERM,
    CHRNOMBBANCOINTERM,
    CHRDIRECBANCOINTERM,
...
SET DECMONTOIMPUESTO_TOTAL = 0;
SET DECTASASEGURIDAD_TOTAL = 0;
SET DECNUMMONTOCOMISION_TOTAL = 0;
...
SET INTOPCIONTC = 1;
ELSE --**SET DecTcCompra = DECTCEURCOMPRA ; 
...
VALUES (
        ...
        NUMTASADECOMPRA,
        NUMTASADEVENTA,
        'ID: ' || TRIM (CAST (NUMIDUNICOFACTURA AS VARCHAR (20))) || ' - Status: ' || TRIM (CAST (INTTIPORESULTADO AS VARCHAR (10))) || ' ' || COALESCE (VARMENSAJERESULTADO, '')
    );
UPDATE BLHCYFILES.TB_SE_TRN_PGTRANSACCIONESFACTURAS
SET ESTADOFACTURA = (
        CASE
...
VALUES (
        ...
        NUMTASADECOMPRA,
        NUMTASADEVENTA,
        'ID: ' || TRIM (CAST (NUMIDUNICOFACTURA AS VARCHAR (20))) || ' - Cuota pendiente Cancelada'
    );
END IF;
END IF;
ELSE
...
--** Acumular rubros contables individuales
SELECT MONTOCOMISION,
    MONTOIMPUESTO,
    TASASEGURIDAD INTO NUMMONTOCOMISION,
    NUMMONTOIMPUESTO,
    NUMTASASEGURIDAD
FROM BLHCYFILES.TB_SE_TRN_PGFACTURASEXTERIOR
WHERE IDUNICOFACTURA = NUMIDUNICOFACTURA;
SET DECMONTOIMPUESTO_TOTAL = DECMONTOIMPUESTO_TOTAL + COALESCE(NUMMONTOIMPUESTO, 0);
SET DECTASASEGURIDAD_TOTAL = DECTASASEGURIDAD_TOTAL + COALESCE(NUMTASASEGURIDAD, 0);
SET DECNUMMONTOCOMISION_TOTAL = DECNUMMONTOCOMISION_TOTAL + COALESCE(NUMMONTOCOMISION, 0);
END FOR FACT_LOOP;
IF DECMONTODEBITO > 0 THEN IF NUMFORMAPAGO_ANT = 3
AND NUMTIPO <> 2 THEN
SET VARDESCRIPCIONDEBITO = SUBSTRING (
...
IF INTTIPORESULTADO = 0 THEN IF VARCELULARBENEFICIARIO <> '' THEN
SET VARMENSAJESMS = LEFT (
        'Pag@Net: ' || TRIM (COALESCE (VARNOMBREPAGADOR, '')) || ' le ha CANCELADO el pago de factura(s) por un monto de ' || COALESCE (CHRMONEDAFACTURA_ANT, '') || ' ' || TRIM (CAST (DECMONTOCREDITO AS VARCHAR (30))) || ' . Para mayor información ingrese a Banc@Net. Gracias.',
        160
    );
--** Enviar Notificacion SMS						 
CALL BLHSRVLIB.SP_SE_INS_SMS (
...
END IF;
END IF;
ELSE --IF IntTipoResultado = - 1 THEN 
