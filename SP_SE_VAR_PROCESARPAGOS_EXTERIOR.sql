...
SET VARMENSAJERESULTADO = BLHSRVLIB.FN_SE_SEL_ERROR (
        LPAD (
            CAST (INTTIPORESULTADO AS VARCHAR (4)),
            4,
            '0'
        )
    );
...
VALUES (
        ...
        NUMTASADECOMPRA,
        NUMTASADEVENTA,
        'ID: ' || TRIM (CAST (NUMIDUNICOFACTURA AS VARCHAR (20))) || ' - Despues obtener fecha: ' || TRIM (CAST (INTTIPORESULTADO AS VARCHAR (10))) || ' ' || COALESCE (VARMENSAJERESULTADO, '') || ' ' || COALESCE (CHRPROSEGUIR, '')
    );
UPDATE BLHCYFILES.TB_SE_TRN_PGTRANSACCIONESFACTURAS
SET ESTADOFACTURA = (
        CASE
...
VALUES (
        ...
        NUMTASADECOMPRA,
        NUMTASADEVENTA,
        'ID: ' || TRIM (CAST (NUMIDUNICOFACTURA AS VARCHAR (20))) || ' - Despues update factura: ' || TRIM (CAST (INTTIPORESULTADO AS VARCHAR (10))) || ' ' || COALESCE (VARMENSAJERESULTADO, '') || ' ' || COALESCE (CHRPROSEGUIR, '')
    );
