...
--======================================================================================= 
		...
		BEGIN 
		    ...
		    CALL BLHSRVLIB.SP_SE_INS_PGLOGPROCESO ( 0, 0, 0, 0, 0, 0, 
		        SUBSTR('SQL STATE 75000 | MSG: ' || COALESCE(VARMENSAJERESULTADO, 'N/A'), 1, 1000));
		...
		BEGIN 
			...
			SET VARMENSAJERESULTADO = BLHSRVLIB . FN_SE_SEL_ERROR ( LPAD ( INTTIPORESULTADO , 4 , '0' ) ) ; 
		    CALL BLHSRVLIB.SP_SE_INS_PGLOGPROCESO ( 0, 0, 0, 0, 0, 0, 
		        SUBSTR('SQL STATE 75002 | MSG: ' || COALESCE(VARMENSAJERESULTADO, 'N/A'), 1, 1000));
		END ; 
		...
		BEGIN
		    ...
		    CALL BLHSRVLIB.SP_SE_INS_PGLOGPROCESO ( 0, 0, 0, 0, 0, 0, 
		        SUBSTR('ERR_SQL | MSG: ' || COALESCE(VARMENSAJERESULTADO, 'N/A'), 1, 1000));
		END;
		...
		IF CHRPROSEGUIR = 'S' THEN 
			...
			FACT_LOOP : FOR CICLOFACTURAS AS FACTURAS CURSOR WITH HOLD FOR 
				...
				AND F . FORMAPAGO = 3 
				ORDER BY F . CTAPAGADOR , F . FECHAPAGO , F . IDSITIO , F . IDUNICOBENEFICIARIO , F . FORMAPAGO , F . MONEDAFACTURA , AA . TIPO 
			DO 
  
				SET NUMIDUNICOFACTURA = IDUNICOFACTURA ; 

				IF NUMCTAPAGADOR_ANT IS NULL OR NUMCTAPAGADOR_ANT <> CTAPAGADOR OR NUMIDBENEFICIARIO_ANT <> IDUNICOBENEFICIARIO OR 
				NUMFORMAPAGO_ANT <> FORMAPAGO OR CHRMONEDAFACTURA_ANT <> MONEDAFACTURA OR NUMTIPO <> TIPO THEN 
					...
					IF DECMONTODEBITO > 0 THEN 
						...
						IF NUMFORMAPAGO_ANT = 3 AND NUMTIPO <> 2 THEN 
							...
							WHERE IDUNICOFACTURA = NUMIDUNICOFACTURA LIMIT 1 ; 
							...
							CALL BLHSRVLIB . SP_SE_VAR_TRANSFERENCIAEXTERIOR ( NUMCIFPAGADOR , CHRIDUNICOFACTURA , NUMCTAPAGADOR_ANT , CHRGLTRANSACCION , CHRMONEDACUENTAPAGADOR , 
							...
							IF INTTIPORESULTADO <> 0 THEN 
								...
								SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT = VARMENSAJERESULTADO ;
							END IF ; 
							 --------------------------- 
					...
					IF NUMCTAPAGADOR_ANT IS NULL OR NUMCTAPAGADOR_ANT <> CTAPAGADOR THEN 
						...
						IF INTSINREGISTROS = 0 THEN 
							IF CIFPAGADOR = DECCIF THEN 
								IF CHRESTADO = 'N' THEN					 
									...
									SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT = 'ERROR: CUENTA INACTIVA' ;
								END IF ; 
							ELSE 
								SET INTCODIGOERROR = 99 ; 
								SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT = 'ERROR: CIF NO COINCIDE' ;
							END IF ; 
						ELSE 
							SET INTCODIGOERROR = 98 ; 
							SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT = 'ERROR 98' ;
						END IF ; 
						...
						IF CHRPROSEGUIR = 'S' THEN 
							...
							IF NUMSALDODISPONIBLECUENTA <= 0 THEN 
								...
								SIGNAL SQLSTATE '75000' SET MESSAGE_TEXT = 'ERROR: SALDO INSUFICIENTE' ;
							END IF ; 
				...
				IF CHRPROSEGUIR = 'S' THEN 
					...
					IF CHRMONEDACTAPAGADOR <> MONEDAFACTURA THEN 
						...
						IF INTOPCIONTC = 1 THEN						 
							...
							FETCH CURTASAINVERTIDO 
							INTO NUMLINEA , NUMMONTOTRANSFERIDO , CHRMONEDAORIGEN , CHROPERACION , NUMTASA , NUMMONTOCONVERTIDO , CHRMONEDADESTINO , NUMTASANEGOCIADAEXCEDIDA , CHRTIPOTASAUTILIZADA ; 
					...
					IF NUMSALDODISPONIBLECUENTA >= DECMONTODEBITOTEMP THEN 
						...
						IF NUMTIPO = 1 THEN 
							...
							IF CHRPROSEGUIR = 'S' THEN	 
								...
								SELECT TIMESTAMP ( DATE ( DIGITS ( CAST ( CNTRDY + 2000 AS NUMERIC ( 4 , 0 ) ) ) || '-' || DIGITS ( CAST ( CNTRDM AS NUMERIC ( 2 , 0 ) ) ) || '-' || DIGITS ( CAST ( CNTRDD AS NUMERIC ( 2 , 0 ) ) ) ) , CURRENT TIME )
								INTO TIMFECHAACTUAL 
								FROM BLHSRVLIB . A_CNTRLCNT ;							 
