...
CALL BLHSRVLIB.SP_SE_INS_PGLOGPROCESO (
    ...
    0,
    SUBSTR(
        'SQL STATE 75000 | MSG: ' || COALESCE(VARMENSAJERESULTADO, 'N/A'),
        1,
        1000
    )
);
...
CALL BLHSRVLIB.SP_SE_INS_PGLOGPROCESO (
    ...
    0,
    SUBSTR(
        'ERR_SQL | MSG: ' || COALESCE(VARMENSAJERESULTADO, 'N/A'),
        1,
        1000
    )
);
...
--*** MODIFICADO: Lógica unificada de cambio de grupo (Control Break)
...
IF DECMONTODEBITO > 0 THEN 
    --*** MODIFICADO: Bloque de procesamiento del grupo anterior
    IF NUMFORMAPAGO_ANT = 3
    AND NUMTIPO <> 2 THEN 
...
--*** MODIFICADO: Captura de ID y validaciones del nuevo grupo
SET NUMIDUNICOFACTURA_ANT = IDUNICOFACTURA;
SET INTSINREGISTROS = 0;
SELECT ACMCUN,
    CASE
...
--*** MODIFICADO: Bloque redundante (Ya procesado en el inicio del loop)
...
END IF;
END IF;
--*** MODIFICADO: Fin de la unificación (Validaciones y variables de control movidas arriba)
SET CHRHORA = REPLACE(
...
--*** MODIFICADO: Bloque redundante consolidado arriba
SET INTSINREGISTROS = 0;
--*** MODIFICADO: Bloque redundante eliminado (Ya procesado arriba)
END IF;
